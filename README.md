...
| Keys the column sample missed are kept | Columns are learned from a sample of rows, so a JSON key that only a few rows carry could be missing, and every value in it was dropped. A new key is added to the schema as text and the schema is sent again; a key that differs from a known one only in case goes into the known spelling, since Redshift would read both as one column. |
