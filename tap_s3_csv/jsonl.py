...
MadKudu addition: some customers deliver files where every line is one JSON
object (for example Couchbase usage exports), not CSV. A table opts in with
"format": "jsonl" in its table spec; CSV stays the default.
"""
...
def _row_iterator(file_stream, options: Dict) -> Generator:
    ...
    not a JSON object raises with the offending line number, so a malformed
    file fails loudly instead of loading a partial day.
    """
